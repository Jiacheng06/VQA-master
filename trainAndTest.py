    ...
    for i in range(trainArgs['epochs']):
        ...
        for batch_idx, (lines, contactmap, properties) in enumerate(train_loader):
            ...
            if trainArgs['use_regularizer']:
                ...
                identity = Variable(
                    identity.unsqueeze(0).expand(train_loader.batch_size, att.size(1), att.size(1))).cuda()
        ...
        if (trainArgs['doSave']):
            torch.save(attention_model.state_dict(),
                       './model_pkl/DUDE/' + trainArgs['saveNamePre'] + '%d.pkl' % (i + 1))
        if (trainArgs['doTest']):
            testArgs = {}
            ...
            print(f"test_result: {testResult}")
