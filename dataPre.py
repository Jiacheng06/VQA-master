...
trainDataSet = pd.read_csv('./change/train_dataset/DUDE-foldTrain3_new.csv', header=None).to_numpy().tolist()   # TODO 训练集
print('get seq-contact dict....')
# seqContactDict = getSeqContactDict(contactPath, contactDictPath)
path_dict = './change/contactMap_new/'
seqContactDict = {pd.read_csv(path_dict + item, header=None).iloc[1, 0]: torch.tensor(pd.read_csv(path_dict + item, header=None).iloc[2:, :].to_numpy().astype(np.float32)).unsqueeze(0) for item in os.listdir(path_dict)} # TODO 序列到concat map字典
print('get letters....')
